...
public class ClusterBootstrapperTest {
    ...
    public void testRotateSaltPasswordSuccess() throws Exception {
        ...
        when(securityConfigService.findOneByStackId(stack.getId())).thenReturn(Optional.of(securityConfig));
        ...
        verify(stackUpdaterService).updateStatus(stack.getId(), DetailedStackStatus.AVAILABLE, CLUSTER_SALT_PASSWORD_ROTATE_FINISHED, "Salt password rotated");
    }
    ...
    public void testRotateSaltPasswordFailure() throws Exception {
        ...
        verify(stackUpdaterService).updateStatusAndSendEventWithArgs(stack.getId(), DetailedStackStatus.SALT_UPDATE_FAILED, CLUSTER_SALT_PASSWORD_ROTATE_FAILED,
                "Failed to rotate salt password", cause.getMessage());
    }
}
