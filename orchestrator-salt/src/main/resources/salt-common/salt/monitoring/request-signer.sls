...
install_request_signer:
  cmd.run:
    - name: "yum install -y cdp-request-signer --disablerepo='*' --enablerepo=cdp-infra-tools; exit 0"
    - onlyif: "test -f /etc/yum.repos.d/cdp-infra-tools.repo && ! rpm -q cdp-request-signer"
{%- endif %}
